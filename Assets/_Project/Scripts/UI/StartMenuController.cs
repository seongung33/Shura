...
{
    private static readonly Color PanelColor = new Color(0.025f, 0.045f, 0.085f, 0.96f);
    ...
    private void ApplyVisualTheme()
    {
        ...
        CreateHeading(menuRoot.transform, "Title", "SHURA", new Vector2(0f, 210f), 68f, PrimaryTextColor);
        subtitleText = CreateHeading(menuRoot.transform, "Subtitle", "함께 버티고, 끝까지 살아남아라", new Vector2(0f, 142f), 27f, new Color(0.68f, 0.8f, 0.9f, 1f));
        CreateAccentLine(menuRoot.transform);
        ...
        foreach (Button button in buttons)
        {
            ...
            if (text == "싱글 플레이")
            {
                StyleButton(button, tmpLabel, legacyLabel, new Vector2(0f, 42f), ButtonColor);
            }
            else if (text == "멀티 플레이")
            {
                StyleButton(button, tmpLabel, legacyLabel, new Vector2(0f, -38f), ButtonColor);
            }
            else if (text == "설정")
            {
                StyleButton(button, tmpLabel, legacyLabel, new Vector2(0f, -118f), ButtonColor);
                button.onClick.RemoveAllListeners();
                button.onClick.AddListener(ShowSettingsNotice);
            }
            else if (text == "게임 종료")
            {
                StyleButton(button, tmpLabel, legacyLabel, new Vector2(0f, -198f), new Color(0.42f, 0.12f, 0.16f, 1f));
                button.onClick.RemoveAllListeners();
