...
2. `NetworkLobbyState`의 캐릭터 ID를 `Main`까지 보존해 주몽 외형·스킬·스탯에 적용하는 코드는 추가됐다. Unity 검증기와 실제 2인 실행으로 양쪽 구성을 확인해야 한다.
3. `NetworkPlayer.prefab`의 `CharacterVisualRoot`와 `JumongVisual` 연결은 추가됐다. 추가 캐릭터 도입 시 동일한 데이터 목록 순서와 프리팹 참조를 유지해야 한다.
