...
> 기준 시각: **2026-08-10 KST**, `origin/develop` `e20c03c`
...
| 영역 | 최신 통합 상태 | 제출 전 확인 |
|---|---|---|
| 15분 싱글 런 | 900초 `StageConfig`, 구간 웨이브, 팀 EXP·레벨업 선택, 장산범 전환 코드·데이터 구현 | 실제 15분 완주·보스 처치·승리/재시작 |
| 영웅 | 주몽·척준경 선택 데이터, 전용 기본공격·스킬·R 필살기, 컷인·대사·효과음 구현 | 두 영웅 싱글 및 실제 2인 피해·쿨다운·성장 회귀 |
| 적·보스·맵 | 적 3종 4프레임 워크, 역할별 크기/색, 엘리트 연출, 장산범 패턴, 장산 숲 픽셀 배경 구현 | 후반 밀도·충돌·성능, 보스 패턴·승리 수동 확인 |
| UI·UX·오디오 | 메뉴·게임 방법·설정·씬 전환, HUD·레벨업·획득·피격·결과 피드백, 메뉴/전투 BGM 구현 | 화면 비율·음량·입력·중복 재생 회귀 |
| 멀티 | Relay 2인 입장·선택·Main 전환, 서버 전투 권한, 척준경 선택/근접 피해 수정 코드 통합 | 생성→참가→전투→퇴장→재입장과 승리/패배 실제 2인 최종 검증 |
| 배포 | Windows 빌드 이력과 GitHub Actions WebGL Pages 워크플로 존재 | 최신 커밋 산출물·공개 링크·다른 기기 검증 |

중요: `StartMenuController`의 최신 `develop` 런타임 경로는 아직 `mugung_logo_transparent`를 가리킨다. 사용자가 제공한 한글 픽셀 **무궁** 로고 교체는 별도 로고 변경 PR의 병합 여부를 확인해야 한다. 로고 개화·페이드·입력 생략 연출 자체는 `e20c03c`에 병합됐다.

아래 1장 이후의 상세 표에는 8월 9일 감사 이력이 일부 남아 있다. 현재 판단은 이 0장과 `10_TEST_PLAN.md`의 8월 10일 증거표를 우선한다.

## 1. 기준 상태

| 구분 | 현재 값 | 의미 |
|---|---|---|
| 현재 브랜치 | 문서 브랜치 기준 `origin/develop` @ `e20c03c` | 2026-08-10 최신 원격을 기준으로 정적 대조 |
...
| 캐릭터 선택 | 주몽 2인 검증·척준경 정적 연결 | 슬롯 선택 ID를 `NetworkPlayerCharacter`가 플레이어와 함께 보존한다. 주몽 외형·체력·이동속도·기본공격·시작 스킬은 Unity 검증기와 2인 실행을 통과했다. 척준경은 선택 초상화, Idle·Walk, Animator와 Visual Prefab을 `CheokJunGyeongData`에 연결했다 | 척준경 카드·Main 실제 실행, 스킬 구성과 장시간 재접속 회귀 |
...
| 레벨업 선택 | 구현·최종 회귀 필요 | 팀 레벨업, 능력치/스킬 선택, 캐릭터별 스킬 풀과 선택 피드백 | 싱글·2인 연속 레벨업과 동기화 수동 검증 |
| 아이템 | 구현·정적 검증 | 서버 공유 Grid Cell 최초 방문 보급품, 30% 단일 드롭, 회복·자석·적 정지·시전 스킬 쿨다운 초기화와 서버 단일 소비 | 실제 2인 경쟁 획득·자석 이동·정지 복구·쿨다운 즉시 재시전 회귀 |
| 결과 흐름 | 코드 통합·정식 회귀 필요 | 패배와 과거 양쪽 `NetworkTest` 복귀 기록, 호스트 재시작, 세션 정리 후 정식 `MultiPlayerEntry` 복귀 코드·프리팹 | 승리·재시작·정식 복귀 실제 2인 검증 |
| Windows 빌드 | 과거 부분 검증 | 8월 2일 레거시 경로 통합 빌드·2인 실행 기록 | `e20c03c` 이후 제출 후보 빌드·새 기기 검증 |
| Web 빌드 | 워크플로 구현·산출물 미검증 | GitHub Actions WebGL Pages 워크플로 존재 | Actions 성공, 공개 링크, Relay·입력·UI·성능 검증 |
...
이 경로의 2인 시작 조건과 로비 선택 캐릭터의 Main 전투 적용은 코드·씬에 연결됐다. 결과 후 `NetworkGameResultActions`의 복귀 대상도 정식 `MultiPlayerEntry`로 통일했으며 호스트 이탈·새 방·장시간 재접속과 결과 복귀 실행 회귀는 남아 있다.
...
11. 척준경 철혈참·파진·회천참·파성참·검풍·혈로·일기당천 7종의 흰 사각형 플레이스홀더를 모두 전용 순백색 알파 FX로 교체했다. Unity Play Mode에서 스킬별 크기·방향·속성색·지속시간을 최종 회귀해야 한다.
12. 척준경 스킬 아이콘 배치 폴더 `Assets/_Project/Art/UI/SkillIcons/`는 준비했지만 7칸 원본 시트가 없어 `SD_*` 7개의 `icon` 연결은 대기 중이다.
