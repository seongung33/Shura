...
| 2026-08-09 | Codex | 진미리 | 공통 영웅 선택 테마에서 빠진 멀티 로비 전용 정보·버튼 마감 | 선택·네트워크 기능은 유지하고 실제 오브젝트 이름 차이만 런타임 테마가 인식하도록 확장 | 협동 로비 제목, 참가 코드·접속 인원·상태 헤더, 게임 시작·로비 나가기 버튼 크기·색·모션·효과음 | 전체 C# 컴파일·`git diff --check`; 실제 2인 시작 조건·양쪽 로비 화면 수동 회귀 필요 | `CharacterSelectVisualTheme.cs` |
| 2026-08-09 | Codex | 진미리 | 두 번째 영웅 추가 시 캐릭터 카드가 아래로 밀려 잘리는 문제 예방 | 해커톤 확정 범위인 영웅 2명에 맞춰 중앙 목록의 고정 1열을 2열로 변경 | 320×390 카드 2열·중앙 정렬, 싱글·멀티 공통 테마 적용, 연속 머지로 생긴 AI 문서 중복 정리 | 전체 C# 컴파일·`git diff --check`; 주몽·척준경 실제 카드 수동 회귀 필요 | `CharacterSelectVisualTheme.cs` |
...
| 2026-08-10 | Codex·OpenAI 이미지 생성 | 팀 | 척준경 캐릭터 선택 카드에 사용할 고정 팔레트 픽셀 초상화 제작 | 팀 제공 척준경 설정화를 정체성 참조로 사용하고 흉상·갑옷·대검·14색 팔레트·투명 배경 조건을 명시 | 512×512 척준경 캐릭터 선택 초상화와 Unity Sprite 임포트 설정. 2차 시안에서 대검을 왼손으로 쥐고 어깨 옆에 세운 자세로 수정 | 크로마키 제거, 최근접 14색 재양자화, 이진 알파 적용. 이미지 크기·모드·색상 집합·투명 모서리를 검사하고 `CheokJunGyeongData` 초상화에 연결. Unity 실제 카드 화면 회귀 필요 | `Assets/_Project/Art/Characters/CheokJunGyeong/Profile/CheokJunGyeong_CharacterSelect_512_v2.png`, `CheokJunGyeongData.asset` |
| 2026-08-10 | Codex·OpenAI 이미지 생성 | 팀 | 척준경 걷기·공격 제작에 사용할 오른쪽 기준 포즈 생성 | 전신 측면·약한 탑다운, 양손 대검 칼끝 아래, 192×192 프레임과 108px 캐릭터 높이, 기존 14색 팔레트를 명시 | 오른쪽을 보는 척준경 단일 대기 스프라이트 | 크로마키 제거, 불투명 경계 기준 108px 최근접 축소, 192×192 투명 캔버스의 공통 바닥선 배치, 14색 재양자화와 이진 알파 검사. Unity 실제 크기·피벗 회귀 필요 | `Assets/_Project/Art/Characters/CheokJunGyeong/CheokJunGyeong_Idle_Right_192.png` |
| 2026-08-10 | Codex·OpenAI 이미지 생성 | 팀 | 척준경의 무거운 오른쪽 보행 8프레임 제작 | 기준 포즈와 설정화를 참조해 접지·다운·패싱·업을 좌우 발로 반복하고 대검·치마판·붉은 술의 작은 흔들림을 명시 | 한 행 8프레임 걷기 시트. 첫 결과의 마지막 칼날 잘림을 발견해 전체 행 축소·좌우 여백 보정본을 다시 생성 | 프레임별 실루엣 분리, 각각 108px 최근접 축소, 192×192 셀의 동일 바닥선 배치, 14색 재양자화, 이진 알파와 1536×192 크기 검사. Unity 애니메이션 재생 회귀 필요 | `Assets/_Project/Art/Characters/CheokJunGyeong/CheokJunGyeong_Walk_Right_8F_192.png` |
| 2026-08-10 | Codex·OpenAI 이미지 생성 | 팀 | 척준경 철혈참의 두 손 수평 베기 8프레임 제작 | 기준 포즈·걷기 시트를 참조해 대기부터 준비·전진·수평 신전·후속·회복을 명시하고 검기·에너지 아크 등 속성 FX를 금지 | 캐릭터와 물리 대검만 포함한 한 행 8프레임 시트. 첫 결과에서 4~6프레임 실루엣이 맞닿아 프레임 분리가 불가능한 문제를 찾아 전체 포즈 축소·간격 보정본을 다시 생성 | 프레임별 분리, 108px 최근접 축소, 192×192 셀 바닥선 배치, 14색 재양자화와 이진 알파 검사. Unity 재생과 별도 `ElementVisuals` FX 동기화는 후속 검증 필요 | `Assets/_Project/Art/Characters/CheokJunGyeong/CheokJunGyeong_CheolhyeolSlash_Right_8F_192.png` |
| 2026-08-10 | Codex | 팀 | 생성된 척준경 초상화·기준 포즈·걷기 시트를 캐릭터 선택과 런타임 Visual에 연결 | 기존 주몽의 AnimationClip·Animator Controller·Visual Prefab 직렬화 계약을 척준경 에셋에 적용 | Idle 1프레임, Walk 8프레임 Sample Rate 8, `IsMoving` 전환 Controller, SpriteRenderer·Animator·CharacterWalkAnimator Visual Prefab, CharacterData 두 필드 연결 | 주몽 에셋과 구조 대조, Sprite·Clip·Controller·Prefab GUID와 CharacterData 참조 정적 검사. Unity가 열려 있어 별도 배치 실행은 하지 않았고 실제 선택·이동 회귀가 남음 | `CheokJunGyeong_Idle.anim`, `CheokJunGyeong_Walk.anim`, `CheokJunGyeong_Animator.controller`, `CheokJunGyeongVisual.prefab`, `CheokJunGyeongData.asset` |
| 2026-08-10 | Codex·OpenAI 이미지 생성 | 팀 | 흰 사각형 플레이스홀더였던 척준경 철혈참 시각 효과 교체 | +X 방향 120도 부채꼴과 순백색 RGB·알파 전용 명암을 지정해 속성 색상 치환과 호환되는 FX 원본 생성 | 초록 크로마키 제거, 256×256 Point 축소, 가시 RGB `#FFFFFF` 강제, 알파 4단계 정리, PPU 256·좌측 중앙 피벗 설정, 철혈참 프리팹 연결 | PNG 크기·RGBA·RGB 단일색·알파 단계, 메타 임포트 값과 프리팹 GUID를 정적으로 검사. Unity Play Mode에서 방향·크기·속성색·잔상 동기화 회귀 필요 | `Assets/_Project/Art/Combat/CheokJunGyeong/FX_CheolhyeolSlash_256.png`, `Skill_CheolhyeolSlash.prefab` |
| 2026-08-10 | Codex·OpenAI 이미지 생성 | 팀 | 척준경 파진의 수평 돌진 궤적 플레이스홀더 교체 | 오른쪽으로 이동하는 2:1 쐐기형 검격, 조밀한 선두·끊어진 꼬리·중앙 절단선과 순백색 알파 표현을 지정 | 초록 크로마키 제거, 512×256 Point 축소, 가시 RGB `#FFFFFF` 강제, 진행 방향에 따른 알파 4단계 정리, PPU 256·중앙 피벗 설정, 파진 프리팹 연결 | PNG 크기·RGBA·RGB 단일색·알파 단계, 메타 임포트 값과 프리팹 GUID를 정적으로 검사. Unity Play Mode에서 이동 중 위치·Lv2 폭·Lv3 거리·속성색 회귀 필요 | `Assets/_Project/Art/Combat/CheokJunGyeong/FX_Pajin_512x256.png`, `Skill_Pajin.prefab` |
| 2026-08-10 | Codex·OpenAI 이미지 생성 | 팀 | 척준경 회천참의 360도 회전 참격 플레이스홀더 교체 | 네 변에 닿는 완전한 원형 링, 한 바퀴 감기는 내부 스트로크, 완전 투명한 중심과 순백색 알파 표현을 지정 | 초록 크로마키 제거, 256×256 Point 축소, 가시 RGB `#FFFFFF` 강제, 반경에 따른 알파 4단계 정리, PPU 256·중앙 피벗 설정, 회천참 프리팹 연결 | PNG 크기·RGBA·RGB 단일색·알파 단계·네 변 접촉·중심 투명, 메타 임포트 값과 프리팹 GUID를 정적으로 검사. Unity Play Mode에서 기본 지름·Lv5 마지막 파동·속성색 회귀 필요 | `Assets/_Project/Art/Combat/CheokJunGyeong/FX_Hwecheon_256.png`, `Skill_Hwecheon.prefab` |
| 2026-08-10 | Codex·OpenAI 이미지 생성 | 팀 | 척준경 파성참의 전방 붕괴 충격 플레이스홀더 교체 | 중앙 내려베기 충격점, 방사형 지면 균열, 외곽 충격파 링, 작은 파편과 순백색 알파 표현을 지정 | 초록 크로마키 제거, 256×256 Point 축소, 가시 RGB `#FFFFFF` 강제, 연결된 균열·링과 분리된 먼지 컴포넌트별 알파 정리, PPU 256·중앙 피벗 설정, 파성참 프리팹 연결 | PNG 크기·RGBA·RGB 단일색·알파 단계·네 변 접촉·중앙 충격점, 메타 임포트 값과 프리팹 GUID를 정적으로 검사. Unity Play Mode에서 전방 1.6 위치·기본 지름·Lv5 두 번째 파동·속성색 회귀 필요 | `Assets/_Project/Art/Combat/CheokJunGyeong/FX_Paseong_256.png`, `Skill_Paseong.prefab` |
| 2026-08-10 | Codex·OpenAI 이미지 생성 | 팀 | 척준경 검풍의 관통 검기 투사체 플레이스홀더 교체 | 오른쪽을 향한 세로 초승달 압축 공기 칼날, 오목한 후면과 왼쪽으로 뻗는 두 줄의 잔상을 지정 | 초록 크로마키 제거, 256×128 Point 축소, 가시 RGB `#FFFFFF` 강제, 본체·주요 잔상 2개·꼬리 분절의 연결 컴포넌트별 알파 정리, PPU 256·중앙 피벗 설정, 검풍 프리팹 연결 | PNG 크기·RGBA·RGB 단일색·알파 단계·세로 점유율·좌우 배치, 메타 임포트 값과 프리팹 GUID를 정적으로 검사. Unity Play Mode에서 진행 방향·크기·관통 2회·속성색 회귀 필요 | `Assets/_Project/Art/Combat/CheokJunGyeong/FX_Geompung_256x128.png`, `Skill_Geompung.prefab` |
| 2026-08-10 | Codex·OpenAI 이미지 생성 | 팀 | 척준경 혈로의 지속 오라 플레이스홀더 교체 | 네 변에 닿는 얇은 외곽 링, 안쪽 칼날 눈금과 캐릭터를 가리지 않는 성긴 회전선을 지정 | 초록 크로마키 제거, 256×256 Point 축소, 가시 RGB `#FFFFFF` 강제, 반경에 따라 외곽·눈금·내부 회전선 알파 정리, PPU 256·중앙 피벗 설정, 혈로 프리팹 연결 | PNG 크기·RGBA·RGB 단일색·알파 단계·네 변 접촉·중앙 투명 비율, 메타 임포트 값과 프리팹 GUID를 정적으로 검사. Unity Play Mode에서 렌더러 알파 0.2·지속시간·Lv3 범위·속성색 회귀 필요 | `Assets/_Project/Art/Combat/CheokJunGyeong/FX_Hyeollo_256.png`, `Skill_Hyeollo.prefab` |
| 2026-08-10 | Codex·OpenAI 이미지 생성 | 팀 | 척준경 일기당천의 6초 폭주 오라 플레이스홀더 교체 | 조밀한 내부 링에서 방사되는 각진 에너지 혀, 외곽 압력 링, 캐릭터가 보이는 중앙 공간을 지정하고 혈로보다 공격적인 밀도를 요구 | 초록 크로마키 제거, 256×256 Point 축소, 가시 RGB `#FFFFFF` 강제, 주 오라와 분리된 충격 스트릭의 연결 컴포넌트·반경별 알파 정리, PPU 256·중앙 피벗 설정, 일기당천 프리팹 연결 | PNG 크기·RGBA·RGB 단일색·알파 단계·네 변 접촉·중앙 투명·혈로 대비 밀도, 메타 임포트 값과 프리팹 GUID를 정적으로 검사. Unity Play Mode에서 렌더러 알파 0.25·6초 지속·폭주 강화·종료 참격·속성색 회귀 필요 | `Assets/_Project/Art/Combat/CheokJunGyeong/FX_Ilgidangcheon_256.png`, `Skill_Ilgidangcheon.prefab` |

### 2026-08-10 최종 폴리시·안정화 기록

| 날짜 | 도구 | 담당 | 목적 | AI 산출물·제안 | 사람의 통합·검증 | 적용 위치 |
|---|---|---|---|---|---|---|
| 2026-08-10 | Codex | 진미리 | 성장·획득·필살기 전투 피드백 강화 | 레벨업 카드 전환, 경험치/필드 아이템 피드백, 캐릭터별 필살기 컷인·대사·효과음 구현안 | 기존 서버 권한과 일시정지 계약을 유지해 통합하고 컴파일·정적 검사를 수행. 실제 싱글/2인 수동 회귀는 테스트 계획에 분리 | `LevelUpPanelPresenter.cs`, `PickupFeedbackPresenter.cs`, `UltimateCutInPresenter.cs`, `GameAudioController.cs` |
| 2026-08-10 | Codex·OpenAI ImageGen·Image-to-Pixel | 진미리 | 일반 적과 장산범의 역할 가독성·이동감·한국 설화 정체성 강화 | 적 역할 시안, 4프레임 워크 시트, 엘리트·장산범 시안과 픽셀 변환 기준 | 팀이 변환본을 선택·제공하고 Point 임포트·프레임 애니메이션·프리팹 역할을 연결. 후반 다수 스폰 성능은 수동 회귀 필요 | `ConceptArt/Jinmiri_FinalPolish`, `Assets/_Project/Art/Enemies/Generated`, 적 프리팹 |
| 2026-08-10 | Codex | 진미리 | 척준경 멀티 선택 불가·근접 피해 0 오류 수정 | 캐릭터 ID 적용 경로와 서버 근접 피해 판정 진단·검증기 | 기존 네트워크 캐릭터 계약을 유지해 수정하고 전용 Editor 검증기를 추가. 실제 두 인스턴스 회귀는 제출 전 수행 | `NetworkPlayerCharacter.cs`, `NetworkCharacterApplicationValidator.cs`, 척준경 전투 코드 |
| 2026-08-10 | Codex | 진미리 | 로고 개화와 시작 전환 연출 | 꽃잎·광륜·페이드·입력 생략을 런타임 UI로 구성하는 구현안 | 씬 YAML 충돌을 피하고 기존 메뉴 컨트롤러에 연결. 최신 한글 픽셀 로고 리소스 교체 여부는 별도 확인 | `StartMenuController.cs`, 메인 메뉴 UI 리소스 |
