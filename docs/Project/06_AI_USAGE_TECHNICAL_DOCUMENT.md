...
| 2026-08-05 | Codex | 진미리 | 기본·폭발 화살의 클라이언트 중복 피해 요청을 제거하고 상대 화면에 같은 투사체를 표시 | 소유자가 스킬 인덱스·발사 위치·방향·속성을 서버에 요청하고, 서버가 프리팹·쿨다운·위치를 검증해 실제 피해 투사체를 생성하며 비서버 화면에는 피해 없는 시각 복제본만 전달하는 중계 구조 | `NetworkSkillCastRelay`, `SkillCastContext.VisualOnly`, `StraightProjectile` 피해 권한 분리와 `NetworkPlayer.prefab` 허용 스킬 목록 연결 | 장산범 팀원 브랜치와 수정 범위를 분리하고 프리팹 component 정의 누락·중복 0, `git diff --check`, Unity 6000.3.20f1 NGO RPC 코드 생성과 전체 스크립트 컴파일 성공을 확인. 실제 Editor·실행 파일 2인 투사체 표시·피해 회귀는 PR 전 수동 검증 대상으로 남김 | `Assets/_Project/Scripts/Network/Player/NetworkSkillCastRelay.cs`, `Assets/_Project/Scripts/Combat/StraightProjectile.cs`, `Assets/_Project/Scripts/Player`, `Assets/_Project/Prefabs/Players/NetworkPlayer.prefab` |
